...
> 新增网盘孤儿备份清理（深度扫描 v2），大幅加固扫描引擎准确性与安全护栏
> 发布日期：2026-08-25 · 最低 Obsidian：1.13.7

---

## 快速摘要

- 新增网盘孤儿备份清理命令，支持候选风险分级、安全回收 / 永久删除双模式
- 深度扫描 v2：支持 `parent-only` / `scoped` / `full-vault` 三档扫描范围，可配置忽略规则与资源预算
- 修复 vault 自身目录 `.obsidian_*` / `.bdnsync_*` 时间戳备份扫描盲区
- 插件基础设施目录改为精确名称白名单硬保护，彻底杜绝误删
- 新增 vault 根目录误删护栏，防止空索引时清空整个网盘 vault

## 升级注意

- **最低 Obsidian 版本提升**：`minAppVersion` 由 `1.4.0` 提升为 `1.13.7`，低于此版本的 Obsidian 将无法加载本插件
- **安全加固**：本版本包含多处阻断级安全修复（插件备份白名单硬保护、vault 根目录误删护栏），建议尽快升级

## 完整变更日志

> 以下内容原样来自根目录 CHANGELOG.md 的 v1.0.2 条目。
