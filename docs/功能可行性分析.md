...
**代码现状**：无 Git 集成。但「变更来源」抽象已有雏形——`FileWatcher`（file-watcher.ts）产出的变更集合与引擎增量入口可复用；`PathFilter` 的 `ALWAYS_EXCLUDE`（`misc.ts:155`）已默认排除 `.git`，不会把版本库自身同步上云。
...
**代码现状**：无。当前 `SyncEngine` 强耦合 `BaiduAdapter`（adapter.ts），尚无后端抽象。
...
- #5.9 Git 增量 / #5.10 局域网 P2P：未实现，待独立探索（已预留「变更源抽象」支撑；`StorageBackend` 占位已随 4.7 第三方存储废弃而移除）。
