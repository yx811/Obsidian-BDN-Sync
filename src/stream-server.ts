...
export class StreamServer {
  ...
  async start(): Promise<void> {
    ...
    // 移动端（Android/iOS）没有 Node 的 http 模块，加载即崩；流式预览在移动端直接降级为下载模式。
    if (!Platform.isDesktop) {
      console.warn('[BDNSync] 当前平台非桌面端，本地流式代理已禁用（预览将回退为下载模式）');
      return;
    }
    // 生成一个一次性 token
  ...
  private corsHeaders(): Record<string, string> {
    // 仅允许本机回环页面跨域读取（Obsidian 桌面端 WebView 的 origin 通常为 'null' /
    // 'http://localhost'）。收紧为具体本机源，避免任意外部网页借泄露的 token 跨域拉取。
    return {
      'Access-Control-Allow-Origin': 'http://127.0.0.1, http://localhost, null',
  ...
  private async handleStream(
    ...
    // 纵深防御：即使 token 经 URL 泄露，也仅允许读取「属于当前 vault 远程索引」的文件，
    // 拒绝拉取用户网盘任意其他文件（token 仅防本机其他进程，归属校验才是真正的安全边界）。
    const owned = await this.plugin
      .verifyStreamFsId(fsId, path)
      .catch(() => false);
    if (!owned) {
      res.writeHead(403, { 'Content-Type': 'application/json', ...cors });
      res.end(JSON.stringify({ error: 'forbidden: fsId not in vault index' }));
      return;
    }
  ...
  private async fallbackDownload(
    ...
    if (range) {
      ...
      // 非法 Range（start > end）：返回 416 而非静默空响应（避免恶意/畸形请求）
      if (start > end) {
        res.writeHead(416, {
          ...cors,
