...
export class BaiduAdapter {
  ...
  async upload(
    ...
    try {
      ...
      // createFile 失败：分片已传到百度侧的 type=tmpfile 临时区，但未被合并为正式文件。
      // 百度网盘未提供「abort uploadid」公开接口，临时分片由其侧 TTL 回收；此处不反复重试
      // 已失效的 uploadid（会持续失败），改为移除内存会话并明确告警，避免死循环与配额缓慢泄漏。
      // 下次同路径上传会重新 precreate 拿到新 uploadid，旧 tmpfile 由百度侧清理。
      this.uploadSessions.delete(relPath);
      console.warn(
        `[BDNSync] 上传合并失败，已放弃会话 ${session.uploadid}（${relPath}）：将重新分片上传。百度侧临时分片由 TTL 回收。(${(e as Error).message})`,
      );
      throw e;
    }
  }
